...
(function () {
  ...
  function renderZigZag(products) {
    ...
    container.innerHTML = products.map((p, index) => {
      ...
      if (imgSrc.startsWith('data:image') && imgSrc.length > 500) {
        // Use clean image file for performance if base64 is huge
        if (p.name.toLowerCase().includes('vanessence')) imgSrc = 'assets/images/vanessence.png';
        else if (p.name.toLowerCase().includes('dynamyst')) imgSrc = 'assets/images/dynamyst.png';
        else if (p.name.toLowerCase().includes('rempah') || p.name.toLowerCase().includes('no.2')) imgSrc = 'assets/images/nusantara_no2.png';
        else if (p.name.toLowerCase().includes('roll-on')) imgSrc = 'assets/images/nusantara_rollon.png';
        else imgSrc = 'assets/images/Nusantara1nobg.png';
      }
      ...
      const detailsHtml = `
        <div class="zigzag-info-col">
          ...
          <!-- Pricing & Dual CTAs (WhatsApp + Cart Button) -->
      ...
      return `
        <article class="product-zigzag-row ${isReverse ? 'reverse' : ''} visible" id="product-item-${p.id}" style="opacity: 1 !important; transform: none !important;">
